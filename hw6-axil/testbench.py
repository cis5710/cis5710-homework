...
async def preTestSetup(dut, insns_or_path):
    ...
    # empty memory before each test
    for i in range (dut.memory.NUM_WORDS.value):
...
def runCocotbTestsProcessor(pytestconfig):
    """run processor tests with AXIL memory"""

    verilog_sources = [ PROJECT_PATH / "DatapathPipelinedAxil.sv" ]
...
# datapath latencies
INSN_LATENCY = 6      # 6 stages in pipeline
MISPRED_LATENCY = 3   # flush F, G and D
LOAD2USE_LATENCY = 1  # use WX bypass
...
#  F: insn in Fetch
#  G: insn in Going to insn memory
#  D: insn in Decode
#  X: insn in Execute
#  M: insn in Memory
#  W: insn in Writeback
...
async def testLui(dut):
    ...
    await ClockCycles(dut.clk, INSN_LATENCY + CHECK_LATENCY)
...
async def testLuiLui(dut):
    ...
    await ClockCycles(dut.clk, INSN_LATENCY + 1 + CHECK_LATENCY)
    assertEquals(0x12345000, dut.datapath.rf.regs[1].value, f'failed at cycle {dut.datapath.cycles_current.value.integer}')
...
async def testLui3(dut):
    ...
    await ClockCycles(dut.clk, INSN_LATENCY + 2 + CHECK_LATENCY)
    assertEquals(0x12345000, dut.datapath.rf.regs[1].value, f'failed at cycle {dut.datapath.cycles_current.value.integer}')
    assertEquals(0x6789A000, dut.datapath.rf.regs[2].value, f'failed at cycle {dut.datapath.cycles_current.value.integer}')
...
async def testWD1(dut):
    ...
    await ClockCycles(dut.clk, INSN_LATENCY + 3 + CHECK_LATENCY)
    assertEquals(42, dut.datapath.rf.regs[2].value, f'failed at cycle {dut.datapath.cycles_current.value.integer}')
...
async def testX0Bypassing(dut):
    ...
    await ClockCycles(dut.clk, INSN_LATENCY + 4 + CHECK_LATENCY)
...
async def testBeqTaken(dut):
    ...
    await ClockCycles(dut.clk, INSN_LATENCY + MISPRED_LATENCY + 2 + CHECK_LATENCY)
...
async def testLoadUse1(dut):
    ...
    await ClockCycles(dut.clk, INSN_LATENCY + LOAD2USE_LATENCY + 1 + CHECK_LATENCY)
...
async def testLoadUse3(dut):
    ...
    await ClockCycles(dut.clk, INSN_LATENCY + LOAD2USE_LATENCY + 2 + CHECK_LATENCY)
    assertEquals(0x0000_2083, dut.datapath.rf.regs[2].value, f'failed at cycle {dut.datapath.cycles_current.value.integer}')
    assertEquals(0xABCD_E000, dut.datapath.rf.regs[3].value, f'failed at cycle {dut.datapath.cycles_current.value.integer}')
    pass

@cocotb.test(skip='RVTEST_ALUBR' in os.environ)
async def testLoadUse4(dut):
    """load to multiple uses"""
    await preTestSetup(dut, '''
        lw x1,0(x0) # loads bits of the lw insn itself
        addi x2,x1,0
        addi x3,x1,1
    ...
    await ClockCycles(dut.clk, INSN_LATENCY + LOAD2USE_LATENCY + 3 + CHECK_LATENCY)
    assertEquals(0x0000_2083, dut.datapath.rf.regs[2].value, f'failed at cycle {dut.datapath.cycles_current.value.integer}')
    assertEquals(0x0000_2084, dut.datapath.rf.regs[3].value, f'failed at cycle {dut.datapath.cycles_current.value.integer}')
    assertEquals(0x0000_2085, dut.datapath.rf.regs[4].value, f'failed at cycle {dut.datapath.cycles_current.value.integer}')
    pass
...
"""
    ...
    await ClockCycles(dut.clk, INSN_LATENCY + LOAD2USE_LATENCY + MISPRED_LATENCY + CHECK_LATENCY + 1)
    assertEquals(0x0000_2083, dut.datapath.rf.regs[1].value, f'failed at cycle {dut.datapath.cycles_current.value.integer}')
    assertEquals(0x0000_2084, dut.datapath.trace_completed_pc.value, f'failed at cycle {dut.datapath.cycles_current.value.integer}')
    pass
...
@cocotb.test(skip='RVTEST_ALUBR' in os.environ)
async def testLoadBranchNotTaken(dut):
    await preTestSetup(dut, '''
        lw x1,0(x0) # loads bits of the lw insn itself
        beq x1,x0,target
        addi x2,x1,1
        ...
        ''')
    await ClockCycles(dut.clk, INSN_LATENCY + 3 + CHECK_LATENCY)
    assertEquals(0x0000_2084, dut.datapath.rf.regs[2].value, f'failed at cycle {dut.datapath.cycles_current.value.integer}')    
    pass

@cocotb.test(skip='RVTEST_ALUBR' in os.environ)
async def testLoadBranchTaken(dut):
    await preTestSetup(dut, '''
        lw x1,0(x0) # loads bits of the lw insn itself
        beq x1,x1,target
        addi x2,x1,1
        ...
        addi x2,x1,4
        ''')
    await ClockCycles(dut.clk, INSN_LATENCY + MISPRED_LATENCY + 3 + CHECK_LATENCY)
    assertEquals(0x0000_2085, dut.datapath.rf.regs[2].value, f'failed at cycle {dut.datapath.cycles_current.value.integer}')    
    pass

@cocotb.test(skip='RVTEST_ALUBR' in os.environ)
async def testWMgone(dut):
    """WM bypass
Timing:
FGDXMW
 FGD*XMW
"""
    ...
    await ClockCycles(dut.clk, INSN_LATENCY + 1 + CHECK_LATENCY)
    mem_value = read32bFromMemory(dut, 12)
...
async def testLoadToStoreAddress(dut):
    ...
    # wait until store is in X stage, just before it writes to memory
    await ClockCycles(dut.clk, INSN_LATENCY)
    mem_value = read32bFromMemory(dut, loadValue)
...
async def testLoadStore(dut):
    ...
    await ClockCycles(dut.clk, INSN_LATENCY + CHECK_LATENCY)
    assertEquals(0x0000_2083, dut.datapath.rf.regs[1].value, f'failed at cycle {dut.datapath.cycles_current.value.integer}')
    mem_value = read32bFromMemory(dut, 0)
    assertEquals(0x0000_2000, mem_value, 'wrong value at address 0x0')
    pass

@cocotb.test(skip='RVTEST_ALUBR' in os.environ)
async def testLoad2Stores(dut):
    """load followed by 2 stores"""
    await preTestSetup(dut, '''
        lw x1,0(x0)
        sb x0,0(x0)
        sb x0,1(x0)
...
async def testDiv(dut):
    ...
    await ClockCycles(dut.clk, INSN_LATENCY + DIVIDER_STAGES + CHECK_LATENCY)
...
async def testDivUse(dut):
    ...
    await ClockCycles(dut.clk, INSN_LATENCY + DIVIDER_STAGES + 1 + CHECK_LATENCY)
    assertEquals(1, dut.datapath.rf.regs[2].value, f'failed at cycle {dut.datapath.cycles_current.value.integer}')
...
async def testDivDivUse(dut):
    ...
    await ClockCycles(dut.clk, INSN_LATENCY + (2*DIVIDER_STAGES) + 2 + CHECK_LATENCY)
...
async def riscvTest(dut, binaryPath=None, tracingMode=None):
    ...
    trace = []
    traceFile = Path(f'trace-{binaryPath.name}.json')
...
RV_TEST_BINARIES = [
    cu.RISCV_TESTS_PATH / 'rv32ui-p-simple', # riscvTest_001
...
async def dhrystone(dut, tracingMode=TRACING_MODE):
    ...
    trace = []
    traceFile = Path(f'trace-{dsBinary.name}.json')
    if tracingMode == 'compare':
        # use ../ since we run from the sim_build directory
        with open(Path('..') / traceFile, 'r', encoding='utf-8') as f:
            trace = json.load(f)
