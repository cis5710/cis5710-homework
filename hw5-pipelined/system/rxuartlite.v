...
`else
		parameter  [(TIMER_BITS-1):0]	CLOCKS_PER_BAUD = 217,	// 115200 Baud at 100MHz
`endif
	...
	//reg	[31:0] wr_counter;
	//reg counting_flag;
	//initial wr_counter = 0;
	//initial counting_flag = 0;
	...
	/*always @(posedge i_clk) begin
		if (i_reset) begin
			...
			if ((zero_baud_counter)&&(state == RXUL_STOP)&&(ck_uart)&&(counting_flag==0)) begin
				o_wr   <= 1'b1;
				o_data <= data_reg;
				counting_flag <= 1;
			end
			if (counting_flag == 1) begin
				wr_counter <= wr_counter + 1;
				if (wr_counter == COUNTER_MAX) begin
					o_wr <= 1'b0;
					wr_counter <= 0;
					counting_flag <= 0;
				end
